...
<Wrapper class="grid grid-cols-1 gap-4 @md:grid-cols-2">
  ...
  <CTA href="https://github.com/odhyp/idx-daily/releases/download/v0.0.2/idxdaily.exe" text="Download (.exe)" icon="material-symbols:terminal" />
</Wrapper>
</Wrapper>
...
2. Create a `config.toml` file (or [download the example](https://github.com/odhyp/idx-daily/blob/main/config.toml))
3. Configure it:
...
5. Use the "**Launch Chrome**" option to start a browser session
6. Choose a start and end date when prompted
7. Review the confirmation table for date range and total workdays
8. Confirm to start the download
9. The resulting Excel files will be saved to your configured `output_dir`
